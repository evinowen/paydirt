...
export async function parseResume(resumePath: string): Promise<ResumeData> {
  ...
  if (skillsIdx !== -1) {
    ...
    data.skills = skillLines.flatMap(l => l.split(/[,•|]/)).map(s => s.trim()).filter(Boolean);
  }
...
function isSectionHeader(line: string): boolean {
  return /^(skills?|experience|education|work|projects?|certifications?|summary|objective)$/i.test(line) &&
    line.length < 50;
}
