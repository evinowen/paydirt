...
export class GlassdoorApplicator extends BaseApplicator {
  ...
  async apply(job: JobPosting, resume: ResumeData): Promise<ApplicationResult> {
    ...
    try {
      ...
      const [newPage] = await Promise.all([page.context().waitForEvent('page'), applyBtn.click()])
      ...
      if (resume.email)
        await this.fillField(newPage, 'input[name="email"], input[type="email"]', resume.email)
      if (resume.phone)
      ...
      return {
        success: true,
        message: 'Application started — external ATS opened, manual completion may be needed',
      }
    } catch (err) {
