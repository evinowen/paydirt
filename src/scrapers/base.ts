...
export abstract class BaseScraper {
  ...
  protected applyFilters(jobs: JobPosting[], options: SearchOptions): JobPosting[] {
    return jobs.filter(job => {
      if (options.exclude_companies?.some(c =>
        job.company.toLowerCase().includes(c.toLowerCase()))) return false;
      if (options.exclude_keywords?.some(k =>
        job.title.toLowerCase().includes(k.toLowerCase()) ||
        job.description.toLowerCase().includes(k.toLowerCase()))) return false;
      return true;
    });
  }
}
