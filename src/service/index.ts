...
export class JobSearchService extends EventEmitter {
  ...
  constructor(config: Config, private resume: ResumeData, configPath: string) {
    super();
    this.state = {
      status: 'idle',
      jobs: [],
  ...
  async runSearch(): Promise<void> {
    ...
    if (uniqueNew.length > 0) {
      ...
      this.log(`Found ${uniqueNew.length} new posting(s). Use "apply all" or "apply <number>" to apply.`);
    ...
    try {
      ...
      switch (job.source) {
        ...
        case 'glassdoor':
          ...
          result = await new GlassdoorApplicator(this.state.config.glassdoor).apply(job, this.resume);
          break;
      }
