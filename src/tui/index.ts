...
export class TUI {
  ...
  constructor(private service: JobSearchService) {
    this.screen = blessed.screen({ smartCSR: true, title: 'Paydirt', fullUnicode: true, dockBorders: true });
    ...
    this.logPanel = blessed.log({
      parent: this.screen,
      top: 1, left: 0, width: '60%', height: '100%-5',
    ...
    this.jobsPanel = blessed.list({
      parent: this.screen,
      top: 1, right: 0, width: '40%', height: '50%-2',
    ...
    this.detailPanel = blessed.box({
      parent: this.screen,
      bottom: 3, right: 0, width: '40%', height: '50%-2',
    ...
    this.statusBar = blessed.box({
      parent: this.screen,
      bottom: 2, left: 0, width: '60%', height: 1,
      tags: true,
  ...
  private wireEvents(): void {
    ...
    this.service.on('jobs:found', () => { this.refreshJobs(); this.screen.render(); });
    this.service.on('jobs:updated', () => { this.refreshJobs(); this.screen.render(); });
    this.service.on('status:change', (state: ServiceState) => { this.renderHeader(state); this.screen.render(); });
  ...
  private renderHeader(state: ServiceState): void {
    const colors: Record<string, string> = {
      idle: '{green-fg}', searching: '{yellow-fg}', applying: '{cyan-fg}', error: '{red-fg}',
    };
